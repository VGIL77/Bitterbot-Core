...
export default function ThreadPage({
  ...
  useEffect(() => {
    if (projectName) {
      document.title = `${projectName} | Bitterbot`;
      ...
      if (metaDescription) {
        metaDescription.setAttribute(
          'content',
          `${projectName} - Interactive agent conversation powered by Bitterbot`,
        );
      }

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | Bitterbot`);
      }
  ...
  useEffect(() => {
    if (initialLoadCompleted && subscriptionData) {
      const hasSeenUpgradeDialog = localStorage.getItem('bitterbot_upgrade_dialog_displayed');
  ...
  const handleDismissUpgradeDialog = () => {
    ...
    localStorage.setItem('bitterbot_upgrade_dialog_displayed', 'true');
  };
        ...
        <div
          ...
          <div className={cn(
            ...
            <ChatInput
              ...
              placeholder={`Ask ${agent ? agent.name : 'BitterBot'} anything...`}
              loading={isSending}
